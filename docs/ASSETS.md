...
| Asset | Notes |
|---|---|
| Action SFX | **Already working, synthesised at runtime** — eleven sounds generated by `Synth`, pitched per character. No files to license. Commission real recordings only if the synthesised set proves thin in user testing; the `SoundID` vocabulary would not change. |
| UI SFX ×3 | Tap, unlock, toggle — one exists, two to go |
...
**Licensing:** the synthesised bank has no licensing exposure at all, which is a real
advantage worth preserving. If any recorded audio is added later, commission it original
or use a clearly-licensed library with commercial rights suitable for a paid app, record
the licence for every file, and avoid anything with attribution-in-app requirements.
